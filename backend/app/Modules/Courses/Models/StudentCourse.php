...
class StudentCourse extends Model{
    ...
    //     }
    //     $source = $source->where('status',1)->withCount('Student')->orderBy('student_count', 'desc')->groupBy('student_id')->get()->take($count);
    ...
    static function getRevenue(){
        $source = self::NotDeleted()->where('paid',1)->where('status',1)->whereHas('Course',function($courseQuery){
            $courseQuery->whereIn('status',[3,4]);
